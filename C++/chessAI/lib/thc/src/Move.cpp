...
bool Move::NaturalIn(ChessRules* cr, const char* natural_in)
{
    ...
    {
        move[i] = natural_in[i];
        if(move[i] == '\0' || move[i] == ' ' || move[i] == '\t' || move[i] == '\r' ||
           move[i] == '\n')
        {
            move[i] = '\0';
            okay = true;
    ...
    if(okay)
    {
        ...
        // Promotion
        if(len >
           2) // We are supporting "ab" to mean Pawn a5xb6 (say), and this test makes sure we don't
            ...
            if(!is_file)
            {
                switch(last)
                {
                    ...
                    case 'R':
                        ...
                        break;
                    case 'b':
                        if(len == 3 && '2' <= move[1] && move[1] <= '7')
                            break; // else fall through to promotion - allows say "a5b" as
                                   // disambiguating
                                   //  version of "ab" if there's more than one "ab" available!
                                   //  Something of an ultra refinement
                    case 'B':
                        promotion = 'B';
                        break;
                    case 'n':
                    ...
                    default:
                        okay = false;
                        break; // Castling and promotions are the only cases longer than 2
                ...
                if(promotion)
                {
                    ...
                    {
                        case '=':
                        case '1': // we now allow '=' to be omitted, as eg ChessBase mobile seems to
                                  // (sometimes?)
                        case '8':
                            break;
                        default:
    ...
    if(okay)
    {
        if(len == 2 && 'a' <= move[0] && move[0] <= 'h' && 'a' <= move[1] && move[1] <= 'h')
        {
            src_file = move[0]; // eg "ab" pawn takes pawn
            dst_file = move[1];
        }
        else if(len == 3 && 'a' <= move[0] && move[0] <= 'h' && '2' <= move[1] && move[1] <= '7' &&
                'a' <= move[2] && move[2] <= 'h')
        {
            src_file = move[0]; // eg "a3b"  pawn takes pawn
            dst_file = move[2];
        }
        else if(len >= 2 && 'a' <= move[len - 2] && move[len - 2] <= 'h' && '1' <= move[len - 1] &&
                move[len - 1] <= '8')
        {
            dst_file = move[len - 2];
            dst_rank = move[len - 1];
    ...
    if(okay)
    {
        if(len > 2)
        {
            if('a' <= move[0] && move[0] <= 'h' && '1' <= move[1] && move[1] <= '8')
            {
                src_file = move[0];
                src_rank = move[1];
            }
    ...
    if(okay)
    {
        ...
        if(src_file && src_rank && dst_file && dst_rank)
        {
            ...
            {
                m = &list.moves[i];
                if((default_piece || piece == cr->squares[m->src]) && src_file == FILE(m->src) &&
                   src_rank == RANK(m->src) && dst_ == m->dst)
                {
                    if(kcastling)
                    {
                        if(m->special == (white ? SPECIAL_WK_CASTLING : SPECIAL_BK_CASTLING))
                            found = m;
                    }
                    else if(qcastling)
                    {
                        if(m->special == (white ? SPECIAL_WQ_CASTLING : SPECIAL_BQ_CASTLING))
                            found = m;
                    }
                    else
                        found = m;
        ...
        else if(src_file && dst_file && dst_rank)
        {
            ...
            {
                m = &list.moves[i];
                if(piece == cr->squares[m->src] && src_file == FILE(m->src) &&
                   /* src_rank  ==   RANK(m->src)  */
                   dst_ == m->dst)
                {
                    found = m;
        ...
        else if(src_file && src_rank && dst_file)
        {
            for(i = 0; i < list.count; i++)
            {
                m = &list.moves[i];
                if(piece == cr->squares[m->src] && src_file == FILE(m->src) &&
                   src_rank == RANK(m->src) && dst_file == FILE(m->dst))
                {
                    found = m;
                    break;
                }
        ...
        else if(src_file && dst_file)
        {
            for(i = 0; i < list.count; i++)
            {
                   ...
                   dst_file == FILE(m->dst))
                {
                    if(enpassant)
                    {
                        if(m->special == (white ? SPECIAL_WEN_PASSANT : SPECIAL_BEN_PASSANT))
                            found = m;
                    }
                    else
                        found = m;
        ...
        else if(dst_rank && dst_file)
        {
            ...
            {
                m = &list.moves[i];
                if(piece == cr->squares[m->src] && dst_ == m->dst)
                {
                    found = m;
                    break;
                }
    ...
    if(okay && found)
    {
        bool found_promotion = (found->special == SPECIAL_PROMOTION_QUEEN ||
    ...
    if(cr->white)
    {
            ...
            else // if ( r == 'x' )
            {
                ...
                if('a' <= g && g <= 'h')
                {
                    ...
                    if('3' <= r && r <= '7')
                    {
                        ...
                        else if(r == '6' && cr->squares[mv.src] == 'P' &&
                                cr->squares[mv.dst] == ' ' && mv.dst == cr->enpassant_target &&
                                cr->squares[SOUTH(mv.dst)] == 'p')
                        {
                            mv.capture = 'p';
        ...
        else
        {
            ...
            switch(f)
            {
                ...
                case 'Q':
                    ray_lookup = queen_lookup; // fall through
                case 'R':
                    if(f == 'R')
                        ray_lookup = rook_lookup; // fall through
                case 'B':
                    if(f == 'B')
                        ray_lookup = bishop_lookup; // fall through
                case 'N': {
                    char piece = f;
                    f = *natural_in++;
                    char src_file = '\0';
                                ...
                                if(piece == 'N')
                                {
                                    ...
                                    for(int probe = 0; !found && probe < 2; probe++)
                                    {
                                        ...
                                        while(!found && nbr_moves--)
                                        {
                                            ...
                                            {
                                                bool src_file_ok =
                                                    !src_file || FILE(src_) == src_file;
                                                bool src_rank_ok =
                                                    !src_rank || RANK(src_) == src_rank;
                                                if(src_file_ok && src_rank_ok)
                                                {
                                                    mv.src = src_;
                                                    if(probe == 0)
                                                        count++;
                                                    else // probe==1 means disambiguate by testing
                                                         // whether move is legal, found will be set
                                                         // if we are not exposing white king to
                                                         // check.
                                                    {
                                                        char temp = cr->squares[mv.dst];
                                                        cr->squares[mv.dst] =
                                                            'N'; // temporarily make move
                                                        cr->squares[src_] = ' ';
                                                        found = !cr->AttackedSquare(
                                                            cr->wking_square,
                                                            false); // bool AttackedSquare( Square
                                                                    // square, bool enemy_is_white
                                                                    // );
                                        ...
                                        if(probe == 0 && count == 1)
                                            found =
                                                true; // done, no need for disambiguation by check
                                    }
                                }
                                    ...
                                    for(int probe = 0; !found && probe < 2; probe++)
                                    {
                                        ...
                                        while(!found && nbr_rays--)
                                        {
                                            ...
                                            while(!found && ray_len--)
                                            {
                                                ...
                                                {
                                                    // Any piece, friend or enemy must move to end
                                                    // of ray
                                                    ptr += ray_len;
                                                    ray_len = 0;
                                                    if(cr->squares[src_] == piece)
                                                    {
                                                        ...
                                                        if(src_file_ok && src_rank_ok)
                                                        {
                                                            ...
                                                            if(probe == 0)
                                                                count++;
                                                            else // probe==1 means disambiguate by
                                                                 // testing whether move is legal,
                                                                 // found will be set if we are not
                                                                 // exposing white king to check.
                                                            {
                                                                char temp = cr->squares[mv.dst];
                                                                cr->squares[mv.dst] =
                                                                    piece; // temporarily make move
                                                                cr->squares[mv.src] = ' ';
                                                                found = !cr->AttackedSquare(
                                                                    cr->wking_square,
                                                                    false); // bool AttackedSquare(
                                                                            // Square square, bool
                                                                            // enemy_is_white );
                                                                cr->squares[mv.dst] =
                                                                    temp; // now undo move
            ...
            else // if ( r == 'x' )
            {
                ...
                if('a' <= g && g <= 'h')
                {
                    ...
                    if('2' <= r && r <= '6')
                    {
                        ...
                        else if(r == '3' && cr->squares[mv.src] == 'p' &&
                                cr->squares[mv.dst] == ' ' && mv.dst == cr->enpassant_target &&
                                cr->squares[NORTH(mv.dst)] == 'P')
                        {
                            mv.capture = 'P';
                                                ...
                                                if(src_file_ok && src_rank_ok)
                                                {
                                                    ...
                                                    if(probe == 0)
                                                        count++;
                                                         ...
                                                         // whether move is legal, found will be set
                                                         // if we are not exposing black king to
                                                         // check.
                                                    {
                                                        char temp = cr->squares[mv.dst];
                                                        cr->squares[mv.dst] =
                                                        ...
                                                        found = !cr->AttackedSquare(
                                                            cr->bking_square,
                                                            true); // bool AttackedSquare( Square
                                                                   // square, bool enemy_is_white );
                                                        cr->squares[mv.dst] = temp; // now undo move
                                                        cr->squares[mv.src] = 'n';
                                                    }
...
{
    cout << " TerseIn" <<tmove<<endl;
    MOVELIST list;
    int i;
    bool okay = false;
    if(strlen(tmove) >= 4 && 'a' <= tmove[0] && tmove[0] <= 'h' && '1' <= tmove[1] &&
       tmove[1] <= '8' && 'a' <= tmove[2] && tmove[2] <= 'h' && '1' <= tmove[3] && tmove[3] <= '8')
    {
    ...
    for(int alg = ALG_PAWN_MOVE; found && !done && alg <= ALG_NB1D2; alg++)
    {
        ...
        for(int i = -1; !done && i < (do_loop ? list.count : 0); i++)
        {
            ...
            if(islower(p))
                p = (char)toupper(p);
            if(!IsEmptySquare(
                   m.capture)) // until we did it this way, enpassant was '-' instead of 'x'
                t = 'x';
            else
                t = '-';
            switch(alg)
            {
                ...
                case ALG_NB1D2: {
                    ...
                    if(t == 'x')
                        sprintf(nmove, "%c%c%cx%c%c", p, FILE(src_), RANK(src_), FILE(dst_),
                                RANK(dst_));
                    else
                        sprintf(nmove, "%c%c%c%c%c", p, FILE(src_), RANK(src_), FILE(dst_),
                                RANK(dst_));
                    break;
                }
            }
